...
def extract_ifg_param(ifg_path: str, wm_file_path: str, dem_path: str, los_path: str, slope_path: str, time: str,
                      lon_min, lat_min, lon_max, lat_max, samp=100, inter=True):
        ...
        if inter:
            ...
            inf_dat = pd.DataFrame(
                np.hstack((np.repeat(date1, len(los.ravel())).reshape(-1, 1)[::samp],
                           ...
                           dem_grid[:, 1].reshape(-1, 1)[::samp],
                           dem.ravel().reshape(-1, 1)[::samp],
                           P[::samp, :],
                           T[::samp, :],
                           e[::samp, :],
                           slope.reshape(-1, 1)[::samp])))
        else:
        ...
        if i == 0:
            ...
            if inter:
                ...
                inf_dat.columns = ['date1', 'date2', 'los', 'ifg', 'Lat', 'Hgt_m'] + \
                                  ...
                                  ['slope']
                inf_dat.to_csv(file_name + '_ifg_PTE_fixed_hgtlvs.csv', index=False)
        else:
            print('Length of data: ', len(inf_dat))
            if inter:
                inf_dat.to_csv(file_name + '_ifg_PTE_fixed_hgtlvs_inter.csv', index=False, mode='a', header=False)
            else:
                inf_dat.to_csv(file_name + '_ifg_PTE_fixed_hgtlvs.csv', index=False, mode='a', header=False)
