...
def extract_inter_param(df, workLoc='', wm_file_path: str = '', date_diff=12, from_scratch=False):
    ...
    for i, day in enumerate(Date):
        ...
        if end > Date[-1]:
            ...
            if not from_scratch:
                PTE_start = df_start[
                    df_start.columns[pd.Series(df_start.columns).str.startswith(('P_', 'T_', 'e_'))]].values
                PTE_end = df_end[df_end.columns[pd.Series(df_end.columns).str.startswith(('P_', 'T_', 'e_'))]].values
            ...
            dat = pd.DataFrame(
                np.hstack((df_start.ID.values.reshape(-1, 1), date.reshape(-1, 1), inter_ZTD.reshape(-1, 1),
                           ...
                           inter_PTE)))
            if i == 0:
                name = ['ID', 'Date', 'inf_ZTD', 'Lat', 'Hgt_m'] + ['P_' + str(i) for i in
                                                                    range(1, len(hgtlvs) + 1)] + [
                           'T_' + str(i) for i in range(1, len(hgtlvs) + 1)] + ['e_' + str(i) for i in
                                                                                range(1, len(hgtlvs) + 1)]
                dat.columns = name
                dat.to_csv(workLoc + "inter_PTE_vert_fixed_hgtlvs.csv", index=False)
            else:
                dat.to_csv(workLoc + "inter_PTE_vert_fixed_hgtlvs.csv", mode='a', index=False, header=False)
