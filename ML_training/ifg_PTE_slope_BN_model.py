...
# dump(scaler_x, 'Scaler/ifg_PTE_MinMax_scaler_x.bin', compress=True)
dump(scaler_y, 'Scaler/ifg_PTE_slope_BN_MinMax_scaler_y.bin', compress=True)
...
# Input layer
model.add(tf.keras.layers.Input(shape=(308,)))
model.add(BatchNormalization())
# Adding first hidden layer
model.add(tf.keras.layers.Dense(units=308, activation=PReLU(), kernel_initializer='he_uniform'))
model.add(BatchNormalization())
# Adding first hidden layer
model.add(tf.keras.layers.Dense(units=308, activation=PReLU(), kernel_initializer='he_uniform'))
model.add(BatchNormalization())
...
plt.savefig('Plots/ifg_PTE_slope_BN_MSE_history.png', dpi=300)
plt.clf()
...
plt.xlabel('No. epoch')
plt.legend(loc="upper left")
plt.savefig('Plots/ifg_PTE_slope_BN_MAE_history.png', dpi=300)

# Saving model
model.save('Model/ifg_PTE_slope_BN_model')
...
fig.savefig('Plots/ifg_PTE_slope_BN_Ob_v_Pred.png', dpi=300)
...
cbar.ax.tick_params(labelsize=10)
fig.savefig('Plots/ifg_PTE_slope_BN_Resid_true.png', dpi=300)
