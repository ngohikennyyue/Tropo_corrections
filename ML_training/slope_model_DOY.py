...
dat = dat.dropna()
X = dat[dat.columns[pd.Series(dat.columns).str.startswith(('DOY', 'Lat', 'Hgt_m', 'P_', 'T_', 'e_', 'Slope'))]]
y = dat[['ZTD']]
...
dump(scaler_x, 'Scaler/Slope_DOY_MinMax_scaler_x.bin', compress=True)
dump(scaler_y, 'Scaler/Slope_DOY_MinMax_scaler_y.bin', compress=True)
...
plot_model(model, 'Plots/slope_DOY_model.png', show_shapes=True)
# Plot history: MSE
...
plt.savefig('Plots/Slope_DOY_model_MSE_history.png', dpi=300)
plt.clf()
...
plt.xlabel('No. epoch')
plt.legend(loc="upper left")
plt.savefig('Plots/Slope_DOY_model_MAE_history.png', dpi=300)

# Saving model
model.save('Model/PTE_fixed_hgtlvs_slope_DOY_model')

# Predict different model
predict = scaler_y.inverse_transform(model.predict(x_test))
true = y_test.values

print_metric(true, predict, 'Slope DOY model')

plot_graphs(true, predict, 'Slope_DOY_model', 'Plots')
