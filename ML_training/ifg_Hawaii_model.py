...
# Read in data
dat = pd.read_csv('../../../InSAR/Large_scale/Hawaii/Hawaii_train_ifg_PTE_fixed_hgtlvs.csv')
dat = dat.dropna()
test_dat = pd.read_csv('../../../InSAR/Large_scale/Hawaii/Hawaii_test_ifg_PTE_fixed_hgtlvs.csv')
...
dump(scaler_x, 'Scaler/ifg_Hawaii_model_MinMax_scaler_x.bin', compress=True)
dump(scaler_y, 'Scaler/ifg_Hawaii_model_MinMax_scaler_y.bin', compress=True)
...
for batch in batchsize:
    ...
    model.add(tf.keras.layers.Dense(units=309, activation=PReLU()))
    # Adding hidden layer
    ...
    plt.savefig('Plots/ifg_Hawaii_model_batchsize_{}_MSE_history.png'.format(batch), dpi=300)
    plt.clf()
    ...
    plt.xlabel('No. epoch')
    plt.legend(loc="upper left")
    plt.savefig('Plots/ifg_Hawaii_model_batchsize_{}_MAE_history.png'.format(batch), dpi=300)

    # Saving model
    model.save('Model/ifg_Hawaii_model_batchsize_{}'.format(batch))
    ...
    print_metric(true, predict, 'ifg_Hawaii_model_batchsize_{}'.format(batch))
    plot_graphs(true, predict, 'ifg_Hawaii_model_batchsize_{}'.format(batch), 'Plots')
