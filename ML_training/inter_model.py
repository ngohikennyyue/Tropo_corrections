...
# Read in data
dat = pd.read_csv('../../GNSS_US/US/inter_PTE_vert_fixed_hgtlvs.csv')
dat = dat.dropna()
X = dat[dat.columns[pd.Series(dat.columns).str.startswith(('Lat', 'Hgt_m', 'P_', 'T_', 'e_'))]]
y = dat[['inf_ZTD']]
...
dump(scaler_x, 'Scaler/interferometric_MinMax_scaler_x.bin', compress=True)
dump(scaler_y, 'Scaler/interferometric_MinMax_scaler_y.bin', compress=True)
...
plt.savefig('Plots/inter_MSE_history.png', dpi=300)
plt.clf()
...
plt.xlabel('No. epoch')
plt.legend(loc="upper left")
plt.savefig('Plots/inter_MAE_history.png', dpi=300)

# Saving model
model.save('Model/inter_PTE_fixed_hgtlvs_model')
...
fig.savefig('Plots/inter_Ob_v_Pred.png', dpi=300)
...
cbar.ax.tick_params(labelsize=10)
fig.savefig('Plots/inter_Resid_true.png', dpi=300)
