...
GOES_dat = GOES_dat[GOES_dat['sigZTD']<0.1]
# X = GOES_dat.iloc[:,7:]
# y = GOES_dat[['ZTD']]
...
x_train = train[['Lat', 'Hgt_m', 'CMI_C08', 'CMI_C09', 'CMI_C10', 'CMI_C12']]
x_test = test[['Lat', 'Hgt_m', 'CMI_C08', 'CMI_C09', 'CMI_C10', 'CMI_C12']]
y_train = train[['ZTD']]
y_test = test[['ZTD']]
...
model.add(tf.keras.layers.Dense(units=64, activation=PReLU(), kernel_initializer='he_uniform'))
# Adding hidden layer
model.add(tf.keras.layers.Dense(units=32, activation=PReLU(), kernel_initializer='he_uniform'))
# Adding hidden layer
model.add(tf.keras.layers.Dense(units=16, activation=PReLU(), kernel_initializer='he_uniform'))
# Adding hidden layer
model.add(tf.keras.layers.Dense(units=8, activation=PReLU(), kernel_initializer='he_uniform'))
# Adding hidden layer
model.add(tf.keras.layers.Dense(units=4, activation=PReLU(), kernel_initializer='he_uniform'))
# Adding hidden layer
...
model.fit(x_train, y_train, batch_size=64, epochs=150, validation_split=0.2, callbacks=[es], verbose=0)
...
plt.savefig('Plots/GOES_MSE_history.png', dpi=300)
plt.clf()
...
plt.xlabel('No. epoch')
plt.legend(loc="upper left")
plt.savefig('Plots/GOES_MAE_history.png', dpi=300)
plt.clf()

# Saving model
model.save('Model/US_WE_PTE_SAC_fixed_standard_hgtlvs_cloud_model')
...
fig.savefig('Plots/GOES_Ob_v_Pred.png', dpi=300)
plt.clf()
...
cbar.ax.tick_params(labelsize=10)
fig.savefig('Plots/GOES_Resid_true.png', dpi=300)
plt.clf()
