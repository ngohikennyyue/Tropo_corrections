...
dump(scaler_x, 'Scaler/Slope_MinMax_scaler_x.bin', compress=True)
dump(scaler_y, 'Scaler/Slope_MinMax_scaler_y.bin', compress=True)
...
plt.savefig('Plots/Slope_model_MSE_history.png', dpi=300)
plt.clf()
...
plt.xlabel('No. epoch')
plt.legend(loc="upper left")
plt.savefig('Plots/Slope_model_MAE_history.png', dpi=300)

# Saving model
model.save('Model/PTE_fixed_hgtlvs_slope_model')

# Predict different model
predict = scaler_y.inverse_transform(model.predict(x_test))
true = y_test.values

print_metric(true, predict, 'Slope model')

plot_graphs(true, predict, 'Slope_model', 'Plots')
