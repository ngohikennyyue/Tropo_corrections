...
X = dat[dat.columns[pd.Series(dat.columns).str.startswith(('Lat', 'Hgt_m', 'date1_', 'date2_'))]]
y = dat[['ifg']]
...
plt.savefig('Plots/ifg_PTE_MSE_history.png', dpi=300)
plt.clf()
...
plt.xlabel('No. epoch')
plt.legend(loc="upper left")
plt.savefig('Plots/ifg_PTE_MAE_history.png', dpi=300)

# Saving model
model.save('Model/ifg_PTE_model')
...
fig.savefig('Plots/ifg_PTE_Ob_v_Pred.png', dpi=300)
...
cbar.ax.tick_params(labelsize=10)
fig.savefig('Plots/ifg_PTE_Resid_true.png', dpi=300)
