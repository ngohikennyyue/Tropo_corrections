...
for i, date in enumerate(date_pairs):
    ...
    plt.savefig('Plots/Hawaii/Ifg_' + date + '.png', dpi=300)
    plt.clf()
    ...
    if i == 0:
        ...
        plt.colorbar()
        plt.savefig('Plots/Hawaii/DEM.png', dpi=300)
    else:
        pass
    # Get Dates and WM
    dem = dem.astype(float)
    ...
    date1_wm1, date1_wm2, minute1 = getWM(date1, 'T16_30_00', wmLoc='../InSAR/Hawaii/weather_model/weather_files/')
    date2_wm1, date2_wm2, minute2 = getWM(date2, 'T16_30_00', wmLoc='../InSAR/Hawaii/weather_model/weather_files/')
    ...
    fig.savefig('Plots/Hawaii/Norm_model_TD_predict_{}.png'.format(date), dpi=300)
    plt.clf()
    ...
    plt.savefig('Plots/Hawaii/Total_Delay_{}.png'.format(date), dpi=300)
    plt.clf()
    ...
    im1 = ax1.imshow(TD, cmap='RdYlBu',vmax= ifg_img.max(), vmin=ifg_img.min())
    ...
    im3 = ax3.imshow(ifg_img - TD, extent=[grid[:, 0].min(), grid[:, 0].max(), grid[:, -1].min(), grid[:, -1].max()], cmap='RdYlBu')
    plt.colorbar(im3, ax= ax3, fraction=0.05, pad=0.2, label='(m)')
    ax3.set_title('Tropo Corrected Ifg {}'.format(date))
    fig.savefig('Plots/Hawaii/Norm_model_Ifg_PredTD_{}.png'.format(date), dpi=300)
    plt.clf()
