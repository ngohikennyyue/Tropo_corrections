...
for i, date in enumerate(date_pairs):
    ...
    dem, dem_grid = Resamp_rasterio('products/Extracted/DEM/SRTM_3arcsec_uncropped.tif', lon_min, lat_min, lon_max,
                                    lat_max, ifg)
    los, _ = Resamp_rasterio('Angle/los.rdr', lon_min, lat_min, lon_max, lat_max, ifg)
    ...
    plt.savefig('Plots/Ifg_' + date + '.png', dpi=300)
    plt.clf()
    ...
    if i == 0:
        ...
        plt.colorbar()
        plt.savefig('Plots/DEM.png', dpi=300)
    else:
        pass
    ...
    Day_1 = np.hstack((dem.ravel().reshape(-1, 1), np.vstack(P1.transpose().reshape((P1.shape[-1] * P1.shape[1], 51))),
                       np.vstack(T1.transpose().reshape((T1.shape[-1] * T1.shape[1], 51))),
                       np.vstack(e1.transpose().reshape((e1.shape[-1] * e1.shape[1], 51)))))
    Day_2 = np.hstack((dem.ravel().reshape(-1, 1), np.vstack(P2.transpose().reshape((P2.shape[-1] * P2.shape[1], 51))),
    ...
    fig.savefig('Plots/Norm_model_TD_predict_{}.png'.format(date), dpi=300)
    plt.clf()
    ...
    plt.savefig('Plots/Total_Delay_{}.png'.format(date), dpi=300)
    plt.clf()

    # Plot Ifg along side with TD (predicted)
    plt.style.use('seaborn')
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 15))
    im1 = ax1.imshow(TD, cmap='RdYlBu')
    ...
    im2 = ax2.imshow(convert_rad(ifg - np.nanmean(ifg), 5.6 / 100),
    ...
    fig.savefig('Plots/Norm_model_Ifg_PredTD_{}.png'.format(date), dpi=300)
    plt.clf()
